...
class AngularSpeed:
    ...
    def __init__(
            ...
            pitchspeed,
            yawspeed):
        """ Initializes the AngularSpeed object """
        self.rollspeed = rollspeed
        self.pitchspeed = pitchspeed
        self.yawspeed = yawspeed

    def __equals__(self, to_compare):
        """ Checks if two AngularSpeed are the same """
        try:
            # Try to compare - this likely fails when it is compared to a non
            # AngularSpeed object
            return \
                ...
                (self.yawspeed == to_compare.yawspeed)

        except AttributeError:
            return False

    def __str__(self):
        """ AngularSpeed in string representation """
        struct_repr = ", ".join([
        ...
        return f"AngularSpeed: [{struct_repr}]"

    @staticmethod
    def translate_from_rpc(rpcAngularSpeed):
        """ Translates a gRPC struct to the SDK equivalent """
        return AngularSpeed(
                
                rpcAngularSpeed.rollspeed,
                
                
                rpcAngularSpeed.pitchspeed,
                
                
                rpcAngularSpeed.yawspeed
                )

    def translate_to_rpc(self, rpcAngularSpeed):
        """ Translates this SDK object into its gRPC equivalent """

        
        
            
        rpcAngularSpeed.rollspeed = self.rollspeed
            
        
        
        
            
        rpcAngularSpeed.pitchspeed = self.pitchspeed
            
        
        
        
            
        rpcAngularSpeed.yawspeed = self.yawspeed
...
class Telemetry(AsyncBase):
    ...
    async def attitude_angular_speed(self):
        """ Generated by dcsdkgen """
        request = telemetry_pb2.SubscribeAttitudeAngularSpeedRequest()
        attitude_angular_speed_stream = self._stub.SubscribeAttitudeAngularSpeed(request)

        try:
            async for response in attitude_angular_speed_stream:
                

            
                yield AngularSpeed.translate_from_rpc(response.attitude_angular_speed)
        finally:
            attitude_angular_speed_stream.cancel()
